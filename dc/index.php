...
if ($selectedGroupId > 0) {
    $upcomingWhere[] = 'ce.group_id = ?';
    $upcomingParams[] = $selectedGroupId;
}
...
<style>
    ...
    /*
     * Upcoming events table (mobile-first)
     */
    .dc-upcoming {
    ...
    .dc-upcoming-header .dc-upcoming-badge {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        min-width: 24px;
    ...
    .dc-upcoming-filter select:focus {
        outline: 3px solid #ffdd00;
        outline-offset: 0;
        border-color: var(--dc-scouts-purple, #7413dc);
        box-shadow: none;
    ...
    .dc-upcoming-table thead {
        background: var(--dc-canvas, #f8fafc);
    }

    .dc-upcoming-table th {
    ...
    .dc-upcoming-table tbody tr:last-child td {
        border-bottom: 0;
    }

    .dc-upcoming-table tbody tr:hover {
        background: #f8f5fc;
    }

    .dc-upcoming-table .dc-upcoming-title {
        font-weight: 700;
        color: var(--dc-scouts-purple-dark, #4d0b93);
    }

    .dc-upcoming-table .dc-upcoming-title a {
        color: var(--dc-scouts-purple-dark, #4d0b93);
        text-decoration: none;
    }

    .dc-upcoming-table .dc-upcoming-title a:hover {
        text-decoration: underline;
    }

    .dc-upcoming-table .dc-upcoming-date {
        white-space: nowrap;
        font-weight: 600;
        font-size: 0.82rem;
    }

    .dc-upcoming-table .dc-upcoming-group {
        font-weight: 600;
        font-size: 0.82rem;
        color: var(--dc-muted, #64748b);
    }

    .dc-upcoming-table .dc-upcoming-status {
        font-size: 0.72rem;
        font-weight: 700;
        text-transform: uppercase;
    ...
    .dc-upcoming-status-approved { color: #00a794; }
    .dc-upcoming-status-submitted,
    .dc-upcoming-status-under_review { color: #d4a300; }
    .dc-upcoming-status-draft { color: var(--dc-muted, #64748b); }
    .dc-upcoming-status-changes_requested { color: #dc2626; }
    ...
    /* On mobile: hide the location column to save space */
    @media (max-width: 599px) {
        .dc-upcoming-table .dc-upcoming-col-location {
            display: none;
        }
    }

    /* On desktop: show upcoming section but less prominently */
    @media (min-width: 1100px) {
        .dc-upcoming {
            margin-bottom: 2rem;
        }
...
<section class="dc-upcoming" aria-labelledby="upcoming-heading">
    <div class="dc-upcoming-header">
        <h2 id="upcoming-heading">Upcoming events</h2>
        <?php if ($upcomingEvents): ?>
            <span class="dc-upcoming-badge"><?= count($upcomingEvents) ?></span>
        <?php endif; ?>
    </div>

    <form class="dc-upcoming-filter" method="get" action="/dc/">
        <label for="upcoming-group">Group:</label>
        <select id="upcoming-group" name="group_id" onchange="this.form.submit()">
        ...
        <?php if (isset($_GET['month'])): ?>
            <input type="hidden" name="month" value="<?= e($_GET['month']) ?>">
        <?php endif; ?>
    </form>

    <?php if ($upcomingEvents): ?>
        <div style="overflow-x: auto;">
            <table class="dc-upcoming-table">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Event</th>
                        <th>Group</th>
                        <th class="dc-upcoming-col-location">Location</th>
                        <th>Status</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($upcomingEvents as $ue): ?>
                        <?php
                        $ueDate = !empty($ue['starts_at']) ? date('D j M', strtotime((string) $ue['starts_at'])) : '—';
                        $ueTime = !empty($ue['starts_at']) ? date('H:i', strtotime((string) $ue['starts_at'])) : '';
                        $ueStatus = (string) ($ue['status'] ?? 'draft');
                        $ueStatusClass = 'dc-upcoming-status-' . $ueStatus;
                        $ueCanManage = dc_index_user_can_manage_event_group($ctx, (int) $ue['group_id']);
                        ?>
                        <tr>
                            <td class="dc-upcoming-date"><?= e($ueDate) ?><br><small><?= e($ueTime) ?></small></td>
                            <td class="dc-upcoming-title">
                                <?php if ($ueCanManage): ?>
                                    <a href="/dc/manage-event.php?id=<?= (int) $ue['id'] ?>"><?= e($ue['title']) ?></a>
                                <?php else: ?>
                                    <?= e($ue['title']) ?>
                                <?php endif; ?>
                            </td>
                            <td class="dc-upcoming-group"><?= e($ue['group_name']) ?></td>
                            <td class="dc-upcoming-col-location"><?= e($ue['location_name'] ?: '—') ?></td>
                            <td class="dc-upcoming-status <?= e($ueStatusClass) ?>"><?= e(str_replace('_', ' ', $ueStatus)) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
